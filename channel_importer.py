...
def channel_importer():
    ...
    @tasks.loop(hours=24)
    async def scheduled_import():
        if not scheduled_options:
            return
        try:
            await do_import(scheduled_options)
        except Exception as e:
            print(f"Auto import error: {e}", type_="ERROR")
    ...
    async def importmsgs(ctx, *, args: str):
        ...
        if args.strip() == "stop":
            if scheduled_import.is_running():
                scheduled_import.cancel()
                await ctx.send("Importación programada detenida.")
            else:
                await ctx.send("No hay importación programada.")
            return
    ...
    async def scheduleimport(ctx, *, args: str):
        ...
        if opts['source_id'] is None or opts['dest_id'] is None:
            await ctx.send("Debes especificar --source y --dest.")
            return
        scheduled_options = opts
        scheduled_import.change_interval(hours=interval)
        if scheduled_import.is_running():
            scheduled_import.cancel()
        scheduled_import.start()
        await ctx.send(f"Importación programada cada {interval}h.")

    @bot.command(name="stopimport", description="Detiene la importación programada.")
    async def stopimport(ctx):
        await ctx.message.delete()
        if scheduled_import.is_running():
            scheduled_import.cancel()
        await ctx.send("Importación programada detenida.")
